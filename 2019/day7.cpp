...
llint amplifier(const vector<llint>& input_data, llint phase_setting, llint input_value) {
...
llint amplificationCircuit(const vector<llint>& input_data, vector<llint> phase_settings) {
    llint input = 0;
    for(int i = 0; i < 5; i++) {
...
llint task1(const vector<llint>& input_data){
    llint lo = 0; 
    llint hi = 5;
    vector<vector<llint>> possible_phase_settings = getPossiblePhaseSettings(lo, hi);
    llint best = 0;
    llint current;
    for (const auto& phase_settings : possible_phase_settings) {
...
llint feedbackLoop(const vector<llint>& input_data, vector<llint> phase_settings) {
    llint input = 0;
    vector<Intcode> amplifiers;
    for (int i = 0; i < 5; i++) {
        amplifiers.emplace_back(input_data);
...
llint task2(const vector<llint>& input_data){
    llint lo = 5; 
    llint hi = 10;
    vector<vector<llint>> possible_phase_settings = getPossiblePhaseSettings(lo, hi);
    llint best = 0;
