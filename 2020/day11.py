        ...
        for i, row in enumerate(data):
            for j, col in enumerate(row):
                if not np.isnan(col):
                    ...
                    sums[i,j] = np.sum(np.nan_to_num(data[row_start:row_end, col_start:col_end], copy=True)) - data[i,j]
        for i, row in enumerate(data):
            for j, col in enumerate(row):
                if not np.isnan(col):
        ...
        for i, row in enumerate(data):
            for j, col in enumerate(row):
                if not np.isnan(col):
                    ...
                    north_east = next((x for x in np.fliplr(data[i-nedim:i, j+1:j+1+nedim]).diagonal()[::-1] if not np.isnan(x)), 0)
                    east = next((x for x in data[i, j+1:] if not np.isnan(x)), 0)
                    sedim = min(data[i+1:, j+1:].shape)
                    south_east = next((x for x in data[i+1:i+1+sedim, j+1:j+1+sedim].diagonal() if not np.isnan(x)), 0)
                    south = next((x for x in data[i+1:, j] if not np.isnan(x)), 0)
                    swdim = min(data[i+1:, :j].shape)
                    south_west = next((x for x in np.fliplr(data[i+1:i+1+swdim, j-swdim:j]).diagonal() if not np.isnan(x)), 0)
                    west = next((x for x in data[i, :j][::-1] if not np.isnan(x)), 0)
                    nwdim = min(data[:i, :j].shape)
                    north_west = next((x for x in data[i-nwdim:i, j-nwdim:j].diagonal()[::-1] if not np.isnan(x)), 0)
